...
function tests(sp) {
  describe("allShortestPaths", function() {
    it("returns 0 for the node itself", function() {
      var g = new Graph();
      g.setNode("a");
      expect(sp(g)).toStrictEqual({ a: { a: { distance: 0 } }});
    });
    ...
    it("uses an optionally supplied weight function", function() {
      ...
      expect(sp(g, weightFn(g))).toStrictEqual({
        a: {
          a: { distance: 0 },
    ...
    it("uses an optionally supplied incident function", function() {
      ...
      expect(sp(g, undefined, function(v) { return g.inEdges(v); })).toStrictEqual({
        a: {
          a: { distance: 0 },
          b: { distance: Number.POSITIVE_INFINITY },
    ...
    it("works with undirected graphs", function() {
      ...
      expect(sp(g, weightFn(g), g.nodeEdges.bind(g))).toStrictEqual({
        a: {
          a: { distance: 0 },
          b: { distance: 1, predecessor: "a" },
          c: { distance: 3, predecessor: "b" },
